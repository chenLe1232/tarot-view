...
export default function CardAnimationPage({
  onNavigate,
}: CardAnimationPageProps) {
  const [animationStage, setAnimationStage] = useState<
    "shuffling" | "revealing" | "complete"
  >("shuffling");
        ...
        <div className="relative w-full h-80 flex items-center justify-center">
          <AnimatePresence>
            {cards.map((card, index) => (
                ...
                animate={{
                  x:
                    animationStage === "shuffling"
                      ? [
                          card.x,
                          card.x + Math.sin(Date.now() / 1000 + index) * 10,
                          card.x,
                        ]
                  ...
                  scale:
                    selectedCardIndex === index
                      ...
                      : 1,
                  opacity:
                    animationStage === "complete" && index !== selectedCardIndex
                      ? 0.3
                      : 1,
                }}
                transition={{
              ...
              >
                <Card
                  className={`w-24 h-36 bg-mystical-gradient border-gold/30 relative overflow-hidden cursor-pointer
                  ${
                    selectedCardIndex === index
                      ? "ring-2 ring-gold animate-glow"
                      : ""
                  }
                `}
                >
